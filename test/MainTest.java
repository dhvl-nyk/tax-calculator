...
public class MainTest
{
	...
	public void testNonImportedProducts()
	{
		...
		assertEquals("Total Sales Taxes ", 1.50, store.getPaymentCounter().getReceipt().getTotalSalesTax() , 0);
		assertEquals("Total amount ", 29.83, store.getPaymentCounter().getReceipt().getTotalAmount(), 0);
	}
	...
	public void testImportedProducts()
	{
		...
		store.getPaymentCounter().billItemsInCart(store.getCart());
		assertEquals("Total Sales Taxes", 7.65, store.getPaymentCounter().getReceipt().getTotalSalesTax() , 0);
		assertEquals("Total amount", 65.15, store.getPaymentCounter().getReceipt().getTotalAmount(), 0);
	}
	...
	public void testGeneralCase()
	{
		...
		store.getPaymentCounter().billItemsInCart(store.getCart());
		assertEquals("Total Sales Taxes", 6.70, store.getPaymentCounter().getReceipt().getTotalSalesTax() , 0);
		assertEquals("Total amount", 74.68, store.getPaymentCounter().getReceipt().getTotalAmount(), 0);
	}
	...
	public void testTaxExemptImportedProducts()
	{
		...
		store.getPaymentCounter().billItemsInCart(store.getCart());
		assertEquals("Total Sales Taxes", 7.45, store.getPaymentCounter().getReceipt().getTotalSalesTax() , 0);
		assertEquals("Total amount payable for all products", 90.69, store.getPaymentCounter().getReceipt().getTotalAmount(), 0);
	}
}
