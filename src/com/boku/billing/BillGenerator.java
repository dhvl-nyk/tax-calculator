...
public class BillGenerator
{
	 ...
	 */
	public double calculateTax(double price, double basicSalesTax, boolean imported) {
		double totalProductTax = taxCalculator.calculateTax(price, basicSalesTax, imported);
	...
	public double calcTotalTax(List<Product> prodList) {
		...
		for(Product product : prodList)
			totalTax += (product != null) ? (product.getTaxedCost() - product.getPrice()) : 0.0;
	...
	public double calcTotalAmount(List<Product> productList) {
		...
		for(Product product : productList)
			totalAmount +=  (product != null) ? product.getTaxedCost() : 0.0;
